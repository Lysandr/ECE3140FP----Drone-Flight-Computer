...
void pid_controller(){
  ...
  pid_i_mem_roll += pid_i_gain_roll * pid_error_temp;	// INTEGRAL GAIN OF ERROR
  if(pid_i_mem_roll > pid_max_roll)pid_i_mem_roll = pid_max_roll; // BOUNDARY CONDITION THIS
  else if(pid_i_mem_roll < pid_max_roll * -1)pid_i_mem_roll = pid_max_roll * -1; // NEGATIVE BOUNDARY
